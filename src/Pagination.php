...
namespace GMSystem\Pagination;

/**
 * Class GMSystem Pagination
 *
 * @author Genilson M. Souza <https://github.com/g4tsbycms>
 * @package GMSystem\Pagination
 */
class Pagination
{
    /** @var int|null */
    ...
    {
        $this->class = $cssClass ?? "Pagination";

        if ($this->rows > $this->limit):
            $Pagination = "<nav class=\"{$this->class}\">";
            $Pagination .= $this->firstPage($fixedFirstAndLastPage);
            $Pagination .= $this->beforePages();
            $Pagination .= "<span class=\"{$this->class}_item {$this->class}_active\">{$this->page}</span>";
            $Pagination .= $this->afterPages();
            $Pagination .= $this->lastPage($fixedFirstAndLastPage);
            $Pagination .= "</nav>";
            return $Pagination;
        endif;
     ...
     * @return Pagination
     */
    private function addGetParams(array $params): Pagination
    {
        $this->params = '';
